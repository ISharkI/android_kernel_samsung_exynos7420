/*
 * drivers/cpufreq/cpufreq_intergalactic.c
 *
 * CPU governor for ARM big.LITTLE architecture (2x4 cores)
 *
...
struct cpufreq_intergalactic_cpuinfo {
	...
	unsigned int last_freq_index;
	unsigned int last_new_freq;
#ifdef CONFIG_PMU_COREMEM_RATIO
	int region;
	int prev_region;
#endif
...
static unsigned int hotplugging_cpu_next_switch[8] = { 
	0, 0, 0, 0, // apollo
	0, 0, 0, 0  // atlas
};

// dynamic cpu hotplugging
...
#define DEFAULT_GO_HISPEED_LOAD 99
	...
	int ntarget_loads;
	/* Enables CPU core-hotplugging */
#define DEFAULT_HOTPLUGGING 0
	int hotplugging;
	/* Stores the last know state of the dynamic hotplugging capability */
	int dm_hotplug_was_disabled;
	/* Minimal amount of cores which have to stay active on the cluster */
#define DEFAULT_HOTPLUG_MIN_IN_CORES 1
	unsigned int hotplug_min_in_cores;
	/* Maximal amount of cores which have to stay active on the cluster */
#define DEFAULT_HOTPLUG_MAX_IN_CORES 4
	unsigned int hotplug_max_in_cores;
	/* Maximal amount of cores which stay active on each cluster */
#define DEFAULT_HOTPLUG_IN_LOAD 65
	unsigned int hotplug_in_load;
	/* Load at which cpugov starts to plug out cores */
#define DEFAULT_HOTPLUG_OUT_LOAD 50
	unsigned int hotplug_out_load;
	/* Minimal timespan in usecs in which a core cannot its state */
#define DEFAULT_HOTPLUG_MIN_PLUG_TIME (100 * USEC_PER_MSEC)
	unsigned int hotplug_min_plug_time;
	/*
...
static unsigned int check_mode(int cpu, unsigned int cur_mode, u64 now)
{
	...
	trace_cpufreq_intergalactic_mode(cpu, total_load,
...
static void disable_hotplugging(struct cpufreq_intergalactic_tunables *tunables)
{
	 ...
	 * disabled right now, enable dm-hotplugging again
	 */
	if (!tunables->dm_hotplug_was_disabled) {
		exynos_dm_hotplug_enable();
	}
	
	tunables->hotplugging = 0;
}
...
static void __cpuinit cpugov_hotplugging_worker(struct work_struct *work)
{
	...
	for (i = 0; i < 8; i++) {	
		...
		if (hotplugging_cpu_states[i]) {
			cpu_up(i);
			pr_info("%s: onlined cpu %d", __func__, i);
		} else {
			cpu_down(i);
...
static void cpufreq_intergalactic_timer(unsigned long data)
{
	...
	unsigned int cpu = 0;
	unsigned int cpu_states[2];
	unsigned int cpu_clust = 0;
#ifdef CONFIG_MODE_AUTO_CHANGE
	...
	cur_cpu = pcpu->policy->cpu;
	cur_cluster = (cur_cpu < 4 ? 0 : 1);
	...
	if (pcpu->policy->cur >= tunables->hispeed_freq &&
		...
		trace_cpufreq_intergalactic_notyet(
	...
	if (tunables->hotplugging) {
		...
		// load the current CPU hotplugging states
		for (cpu = 0; cpu < 8; cpu++) {
			...
			cpu_states[cpu_clust] += (cpu_online(cpu) ? 1 : 0);
		}
		...
		for (cpu = 0; cpu < 8; cpu++) {
			...
			if (cpu_clust != cur_cluster) {
				continue; // not the current cluster
			}
			...
			if (cpu_load >= tunables->hotplug_in_load) {
				...
				// check if core-count on cluster would exceed
				if (cpu_states[cpu_clust] + 1 > tunables->hotplug_max_in_cores)
					return;
				...
				// check if core-count on cluster would exceed
				if (cpu_states[cpu_clust] - 1 < tunables->hotplug_min_in_cores)
					continue;

				// offline core
				hotplugging_cpu_states[cpu] = 0;
				hotplugging_cpu_next_switch[cpu] = ktime_now + tunables->hotplug_min_plug_time;
			}
	...
	pcpu->last_freq_index = index;
	pcpu->last_new_freq = new_freq;
	...
	if (pcpu->target_freq == new_freq &&
		...
		trace_cpufreq_intergalactic_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		spin_unlock_irqrestore(&pcpu->target_freq_lock, flags);
	...
	trace_cpufreq_intergalactic_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
static int cpufreq_intergalactic_speedchange_task(void *data)
{
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			trace_cpufreq_intergalactic_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
		...
		const char *buf, size_t count)
{
	...
	tunables->hotplugging = STORE_AS_BOOL(val);

	if (tunables->hotplugging) {
		/*
		 * Store the last know state of the dynamic hotplugging and
		 * disable it so it does not interfere with our hotplugging 
		 */
		tunables->dm_hotplug_was_disabled = exynos_dm_hotplug_disabled();
		exynos_dm_hotplug_disable();
	} else {
		/*
		 * If dynamic hotplugging was not disabled and cpugov-hotplugging was
		...
		const char *buf, size_t count)
{
	...
	tunables->hotplug_min_in_cores = val;
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	tunables->hotplug_max_in_cores = val;
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	// check if value is a valid load
	if (val < 0 || val > 100)
		return count;

	tunables->hotplug_in_load = val;
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	tunables->hotplug_min_plug_time = val;
	return count;
}
...
show_store_gov_pol_sys(hotplugging);
show_store_gov_pol_sys(hotplug_min_in_cores);
show_store_gov_pol_sys(hotplug_max_in_cores);
show_store_gov_pol_sys(hotplug_in_load);
show_store_gov_pol_sys(hotplug_out_load);
show_store_gov_pol_sys(hotplug_min_plug_time);
...
gov_sys_pol_attr_rw(hotplugging);
gov_sys_pol_attr_rw(hotplug_min_in_cores);
gov_sys_pol_attr_rw(hotplug_max_in_cores);
gov_sys_pol_attr_rw(hotplug_in_load);
gov_sys_pol_attr_rw(hotplug_out_load);
gov_sys_pol_attr_rw(hotplug_min_plug_time);
...
static struct attribute *intergalactic_attributes_gov_sys[] = {
	...
	&hotplug_max_in_cores_gov_sys.attr,
	&hotplug_in_load_gov_sys.attr,
	&hotplug_out_load_gov_sys.attr,
	&hotplug_min_plug_time_gov_sys.attr,
...
static struct attribute *intergalactic_attributes_gov_pol[] = {
	...
	&hotplug_max_in_cores_gov_pol.attr,
	&hotplug_in_load_gov_pol.attr,
	&hotplug_out_load_gov_pol.attr,
	&hotplug_min_plug_time_gov_pol.attr,
...
static int cpufreq_governor_intergalactic(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (!tuned_parameters[policy->cpu]) {
			...
			tunables->hotplugging = DEFAULT_HOTPLUGGING;
			tunables->hotplug_min_in_cores = DEFAULT_HOTPLUG_MIN_IN_CORES;
			tunables->hotplug_max_in_cores = DEFAULT_HOTPLUG_MAX_IN_CORES;
			tunables->hotplug_in_load = DEFAULT_HOTPLUG_IN_LOAD;
			tunables->hotplug_out_load = DEFAULT_HOTPLUG_OUT_LOAD;
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERGALACTIC
static
#endif
struct cpufreq_governor cpufreq_gov_intergalactic = {
	.name = "intergalactic",
	.governor = cpufreq_governor_intergalactic,
						...
						unsigned long val, void *v)
{
	...
	trace_cpufreq_intergalactic_cpu_min_qos(cpu, val, pcpu->policy->cur);
						...
						unsigned long val, void *v)
{
	...
	trace_cpufreq_intergalactic_cpu_max_qos(cpu, val, pcpu->policy->cur);
						...
						unsigned long val, void *v)
{
	...
	trace_cpufreq_intergalactic_kfc_min_qos(0, val, pcpu->policy->cur);
						...
						unsigned long val, void *v)
{
	...
	trace_cpufreq_intergalactic_kfc_max_qos(0, val, pcpu->policy->cur);
...
static int __init cpufreq_intergalactic_init(void)
{
	...
	// initialize hotplugging-worker
	cpugov_hotplugging_worker_wq = alloc_workqueue("cpugov_hotplugging_worker_wq", WQ_HIGHPRI, 0);
	if(!cpugov_hotplugging_worker_wq)
		pr_info("cpugov hotplugging workqueue init error\n");
	INIT_WORK(&cpugov_hotplugging_worker_work, cpugov_hotplugging_worker);
...
#endif
	return cpufreq_register_governor(&cpufreq_gov_intergalactic);
}
